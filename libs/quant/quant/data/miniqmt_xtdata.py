...
def interval_to_xt_period(interval: str) -> str:
    ...
    if p not in _XT_PERIODS:
        raise ValueError(f"Unsupported interval '{interval}'. Supported intervals: {sorted(_XT_PERIODS)}")
    return p
...
def _end_exclusive_to_xt(end_date: Optional[str]) -> str:
    ...
    try:
        ...
        raise ValueError(f"Invalid end_date '{end_date}'. Expected format: YYYY-MM-DD") from None
...
def fetch_miniqmt_bars(
    ...
    if use_mock:
        dates = pd.date_range(start=start_date, end=end_date or "2024-12-31", freq="D")
        return pd.DataFrame({
